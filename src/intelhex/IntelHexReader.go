...
func (self *IntelHexReader) iterateLine(line string, fun IteratorFunc) error {
	if len(line) > 11 && line[0] == ':' {
		var byte_count, address, record_type uint64
		var err error
		byte_count, err = strconv.ParseUint(line[1:3], 16, 8)
		if err != nil {
			return err
		}

		address, err = strconv.ParseUint(line[3:7], 16, 16)
		if err != nil {
			return err
		}

		record_type, err = strconv.ParseUint(line[7:9], 16, 8)
		if err != nil {
			return err
		}
